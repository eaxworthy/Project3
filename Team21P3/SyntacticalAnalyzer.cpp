...
*            [Modifications by Elizabeth Axworthy and Alexandria Labossiere]   *
*                                                                              *
* Date: 12/12/19                                                               *
...
* c++ code is output to a file named <input filename>.cpp                      *
*******************************************************************************/
...
int SyntacticalAnalyzer::stmt_list (){
  ...
  if(token == RPAREN_T){
    //trim off last arithmetic operator if an extra one was put in
    if(op.compare("") != 0)
    ...
    //I think a stack would be the better solution to keep track of nested
    // operations, for for sake of time I'm going to use a local variable
    string local_op = op;
    errors += stmt();
    op = local_op;
    if(local_op.compare("") != 0){
      code += local_op;
      op = "";
    }
    else if (code != "")
...
int SyntacticalAnalyzer::stmt (){
  ...
  if(token == NUMLIT_T || token == STRLIT_T || token == SQUOTE_T){ //7  <stmt> -> <literal>
    ...
    debug2 << lex->GetTokenName(token) << " " << lex -> GetLexeme()<<endl;
    token = lex->GetToken();
    //if(token == IDENT_T && code.substr(0, 4).compare("cout") != 0)
      //code += "__RetVal = ";
    errors += action();
    if(token = RPAREN_T){
      token = lex->GetToken();
...
int SyntacticalAnalyzer::any_other_token (){
  ...
  int errors = 0;
  //This does some editing of code to remove any " or ' that were inside the
  //quoted literal which we would want to get rid of before passing to Object
  //following the test cases.

  if(token == STRLIT_T)
    code += lex->GetLexeme().substr(1, lex->GetLexeme().length()-2) + " ";
  else
    code += lex->GetLexeme()+ " ";
  if(token == SQUOTE_T)
      code = code.substr(0, code.length()-2);
